...
- **23 of 24 matrix scenes** ship at bit-perfect parity: `axes`, `grid`, `sphere`, `multi`, `tinted`, `helloWorld`, `multipleSplats`, `debugColor`, `viewer`, `depthOfField`, `nonLod`, `glsl`, `dynamicLighting`, `extSplats`, `animatedWarp`, `splatDissolve`, `splatReveal`, `sogs`, `raycasting`, `splatShaderEffects`, `splatShaderEffectsFlare`, `interactiveDeform`, `interactiveRipples` (three Tier 7 scenes + the Tier 5 full effect bundle with two different effectType branches shipped). The pipeline handles every Spark feature path in the catalogue — accumulator generation, sort, ordering upload, MRT extSplats output, dyno-driven shader chunks, time uniform, custom shader injection, SplatPager streaming, SOGS unpack, multi-instance SplatMesh groups, per-splat custom dyno modifiers with varied gate mechanisms (smoothstep distance gate, runtime `if`-conditional, integer-typed `effectType` dispatch across multiple branches), and large `globals()` blocks with 9+ helper functions.
